...
SOURCES_SHA1_INDEX = "ux_sources_pdf_sha1"


def sources_sha1_index_exists(conn) -> bool:
    with conn.cursor() as cur:
        cur.execute("SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname=%s",
                    (SOURCES_SHA1_INDEX,))
        return cur.fetchone() is not None
...
def ensure_schema(conn) -> dict[str, list[str]]:
    ...
    with conn.cursor() as cur:
        ...
        # batch_ingest.SCHEMA_DDL. Legacy deployments created this table with
        # `pdf_filename text UNIQUE`; the statements below migrate them in
        # place (all idempotent): drop the filename uniqueness, add the sha1
        # uniqueness.
        cur.execute(
        ...
        cur.execute("ALTER TABLE sources DROP CONSTRAINT IF EXISTS sources_pdf_filename_key")
        # Duplicate sha1 rows would block the unique index; keep the earliest.
        cur.execute(
            "DELETE FROM sources s USING sources t "
            "WHERE s.pdf_sha1 = t.pdf_sha1 AND s.id > t.id"
        )
        cur.execute(
            f"CREATE UNIQUE INDEX IF NOT EXISTS {SOURCES_SHA1_INDEX} "
            "ON sources (pdf_sha1) WHERE pdf_sha1 IS NOT NULL"
        )
    conn.commit()
    return added
...
def check_schema(conn) -> None:
    ...
    elif not sources_sha1_index_exists(conn):
        problems.append("sources is not keyed on pdf_sha1 (legacy pdf_filename key)")
    if problems:
        raise RuntimeError(
...
def record_source(conn, pdf_path: Path, sha1: str,
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            "VALUES (%s, %s, %s, %s, %s) "
            "ON CONFLICT (pdf_sha1) WHERE pdf_sha1 IS NOT NULL DO NOTHING",
...
def promote_review_queue(conn, path: Path) -> int:
    ...
    Same grain + guards as the SQLite version: full dedup grain incl. section,
    and only rows whose status is not already 'ok'.
    """
    promoted = 0
