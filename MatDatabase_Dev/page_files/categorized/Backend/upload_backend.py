...
def call_gemini_from_bytes(pdf_bytes: bytes, filename: str) -> Optional[Dict[str, Any]]:
    ...
    "- manufacturer (company or organization; write '' if not provided)\n\n"
    "Extract ALL properties across categories (Mechanical, Thermal, Electrical, "
    "Physical, Optical, Rheological, etc.) and return them as 'mechanical_properties'.\n\n"
    "For each property extract:\n"
    "- property_name\n"
    "- value (or range)\n"
    "- unit\n"
    "- english (converted units e.g. psi, F, inches; write '' if not provided)\n"
    "- test_condition\n"
    "- comments (notes, footnotes, standards; write '' if none)\n\n"
    "All fields are REQUIRED. Respond ONLY with valid JSON following the schema."
    )
